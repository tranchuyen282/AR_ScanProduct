...
public class AnimationsManager : MonoBehaviour
{
    ...
    GameObject mAugmentationObject;
    bool mIsTracking;
    bool mDoAnimationTo2D;
    bool mDoAnimationTo3D;
    bool mIsShowingOverlay;
    ...
    void Start()
    {
        // Determine how far the Canvas should be from Camera in 2D mode
        float uiDistanceFromCamera = (Screen.dpi > 260) ? 550f : 700f;

        // Set our 2D mode location transform to be specified distance away
        transform.localPosition = new Vector3(0, 0, uiDistanceFromCamera);
    }
    ...
    void Update()
    {
        ...
        if (mDoAnimationTo2D)
        {
            ...
            mAugmentationObject.transform.position = Vector3.Lerp(mAugmentationObject.transform.position,
                                                                  transform.position,
                                                                  Time.deltaTime * 5.0f);

            mAugmentationObject.transform.rotation = Quaternion.Slerp(mAugmentationObject.transform.rotation,
                                                                      transform.rotation,
                                                                      Time.deltaTime * 5.0f);

            // Checks for object distance to check animation finish
            if (Vector3.Distance(mAugmentationObject.transform.position, transform.position) < 0.001f)
            {
                mDoAnimationTo2D = false;
            }
        }
        ...
        {
            // Updates AugmentedObject position and rotation in every frame
             mAugmentationObject.transform.localPosition = Vector3.Lerp(mAugmentationObject.transform.localPosition,
                                                                        new Vector3(0, 0, 0.25f),  // set up location of canvas info
                                                                        Time.deltaTime * 5.0f);
           
            mAugmentationObject.transform.localRotation = Quaternion.Slerp(mAugmentationObject.transform.localRotation,
                                                                           Quaternion.identity,
                                                                           Time.deltaTime * 5.0f);
    ...
    public void PlayAnimationTo3D(GameObject augmentedObject)
    {
        ...
        if (mIsShowingOverlay)
        {
            ...
            augmentedObject.transform.position = transform.position;
        }
